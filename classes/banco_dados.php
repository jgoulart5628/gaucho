...
class banco_dados
{
    ...
    public function __construct($banco)
    {
        ...
        if (!is_array($this->cfg)) {
            return $this->erro_sql('Erro nos Parâmetros de Conexão, verifique! ', $this->cliente, $this->tipo);
        }
        ...
        $this->options = [PDO::NULL_NATURAL => true, PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION, PDO::ATTR_PERSISTENT => true];
        try {
            switch ($this->tipo) {
                case 'ORACLE':
...
//                    $this->conn = new PDO("oci:dbname=$this->db", $this->dbuser, $this->dbpasswd, $this->options);
    ...
    public function banco_query($query, $mode = 'sql', $tela = '')
    {
        if (is_object($this->conn)) {
            ...
            try {
                ...
                $msg = $e->getMessage();
                $info = $e->getCode();
                $this->erro_sql($msg, $query, $info, $tela);
    ...
    public function banco_query_blob($query, $column1, $blob_data1, $column2 = '', $blob_data2 = '', $tela = '')
    {
        if (is_object($this->conn)) {
            $this->conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $this->conn->setAttribute(PDO::ATTR_ORACLE_NULLS, PDO::NULL_NATURAL);
            try {
                ...
                if ($column2) {
                    $this->sql->bindValue($column2, $blob_data2, PDO::PARAM_LOB);
                }
                $this->sql->execute();
            } catch (PDOException $e) {
                $msg = $e->getMessage();
