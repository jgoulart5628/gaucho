...
<html>
  <head>
    ...
    <style>
      ...
      .dropdown-item:hover   {  display: block; }
      .navbar a:hover, .dropdown:hover { background-color: blue; }
</style>
...
//    list-unstyled components
    ...
    for ($i = 0; $i < count($result); ++$i) {
        ...
        if ($sistema < 99) {
            if ($sis != $sis_ant) {
                if ($i > 0) {
                    $ret .= '</div></div><div class="dropdown">';
                }
//                <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false">
                //item da lista do menu (sistema).
                $ret .= '<button class="dropbtn" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false"><i class="fa fa-folder-open"></i>
                 &nbsp;'.$sis.'<i class="fa fa-caret-down"></i>
                </button>
...
//            $ret .= '<a class="dropdown-item" href="javascript:void()" onclick="Saida('.$arq.'); return false;"><i class="fa fa-edit"></i>'.$nome_menu.'</a>';
            ...
            //          current_page_item>
            $ret .= '<a  href="#" onclick="Saida('.$arq.'); return false;">'.$nome_menu.'</a>';
        }
    }
    $ret .= '</div></div>';
...
//    list-unstyled components
    ...
    for ($i = 0; $i < count($result); ++$i) {
        ...
        if ($sistema < 99) {
            if ($sis != $sis_ant) {
                if ($i > 0) {
                    $ret .= '</div></li><br><li class="nav-item dropdown">';
                }

                //item da lista do menu (sistema).
                $ret .= '<a class="nav-link mx-auto" href="#" id="'.$sis.'"  data-toggle="dropdown" aria-expanded="false" aria-haspopup="true">
