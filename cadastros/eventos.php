...
function Manut_CRUD($id=0, $oper='I')
{
   ...
   // tela de abas                   <br class="panel-body" style="background-color: #7FDBFF;">
...
function tela_modalidades($evento_id, $titulo_evento = '', $resp)  {
    ...
    */    
    ...
    <form id="tela_modal" name="tela_modal"  role="form" method="POST">
     ...
     </div><div id="modalidades">'.Tela_modal($evento_id, $resp).'</div>';
    return $tela;
    }

function Tela_modal($evento_id, $resp)  {
  global $db;
  $telax = '<div class="row">
             <div class="form-group col-xs-12 col-sm-6">
             ...
             if (count($resul) > 0) {
              foreach ($resul as $mod) {
               ...
               $telax .= '<input type="text" class="form-control" value="'.$nome_modal.'" readonly>';
              } 
           } 
  $telax .= '<button type="submit" class="btn btn-danger" onclick="xajax_Monta_Modal(xajax.getFormValues(\'tela_modal\')); return false;">Atualiza Modalidades</button>
            </div>
...
function Monta_Modal($dados) {
    ...
    $telaz = '<label>Lista de Modalidades: </label>
               <input type="hidden" name="evento_id" value="'.$evento_id.'">
               ...
               <div class="lista_mod form-group">';
   for ($a = 0; $a < count($res); $a++)  {
      $sel    = $res[$a]['selected'];
      if ($sel == "selected") { $check = ' checked '; } else { $check = '';}
      $mod_id = $res[$a]['mod_id'];
      $descri = $res[$a]['descri'];
      $telaz .= '<input type="checkbox" name="modal[]" value="'.$mod_id.'" '.$check.'>&nbsp;'.$descri.'</br>';
    }
    $telaz .=  '</div>';
...
//    $lista = $dados['lista'];
//    $resp->alert('Aqui : '.print_r($dados,true)); return $resp; 
    if (count($dados['modal']) > 0) {
      $res = $db->Atualiza_modal_evento($evento_id, $dados, $resp);
    }
    $resp->assign('atu_modal', "innerHTML", '');
    $resp->assign('modalidades', "innerHTML", '');
    $tela = Tela_modal($evento_id, $resp);
    $resp->assign('modalidades', 'innerHTML', $tela);
    return $resp;
}
